...
- **Cursor-based tail subscriptions** (C-1) — `store.subscribe_from(cursor)` returns a `TailSubscription` that streams entries past the cursor via `next_batch(timeout_ms, max_count)`. The oplog is the buffer; no per-subscriber in-memory queue. Kafka-style semantics: resumable across reconnects, no drop policy, no backpressure on the producer. Works for both local appends and entries arriving via `merge_sync_payload`. Stale cursors (after compaction) raise `ValueError`; `register_subscriber_cursor(cursor)` blocks compaction while the subscriber is behind. Measured producer-side overhead (1000 appends, median of 3 runs): 0 subscribers 3.12 ms (baseline), 1 subscriber 4.1 ms, 10 subs 4.8 ms, 100 subs 4.6 ms — overhead plateaus at ~30-55%, doesn't scale with subscriber count (Condvar::notify_all is O(1)). Wake-up latency: p50 0.07ms, p99 0.10ms. See [FAQ.md](FAQ.md#how-do-i-tail-silks-oplog-like-kafka) and `examples/tail_subscription.py`.
- **Notify strategy API** — `store.set_notify_strategy(...)` lets users choose between `ImmediateNotify` (default, lowest latency) and `CoalescedNotify(min_interval_ms)` (skip notifies within a time window). Strategies carry a `__silk_strategy__` marker and are evaluated in Rust for zero FFI cost per append. **Honest note:** benchmarking showed coalescing does NOT measurably reduce producer throughput in typical workloads — the active-subscriber overhead is dominated by Python GIL scheduling, not notify frequency. API is kept for subscriber-heavy workloads where fewer, larger wakes reduce per-wake processing cost. Default is `Immediate`.
