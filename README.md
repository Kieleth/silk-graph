...
- **Bundled agent skills** — `silk-skills install --user` installs two Claude Code skills that ship inside the wheel: `silk-graph` (using the library: schema evolution, quarantine diagnosis, compaction, sync) and `silk-internals` (changing it: wire format rules, invariants, bug classes). No clone required — they travel with `pip install silk-graph`.
